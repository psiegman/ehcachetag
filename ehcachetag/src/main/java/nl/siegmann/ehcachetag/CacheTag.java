...
public class CacheTag extends BodyTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		this.cacheLocation = createCacheLocation();
		
		if (cacheLocation == null || cacheLocation.getCacheKey() == null) {
			// no cacheKey: generate and write body content in the normal way
			return BodyTagSupport.EVAL_BODY_INCLUDE;
		}
		
		String cachedBodyContent = getCachedBodyContent(cacheName, cacheLocation);
	 ...
	 */
	CacheLocation createCacheLocation() {
		...
		if (cacheKeyMetaFactory == null) {
			...
			if (key == null) {
				...
				return new CacheLocation(key, cacheName);
			}
		}
		
		// let the cacheKeyMetaFactory determine the cacheKey
		CacheLocation result = null;
		CacheKeyFactory cacheKeyFactory = cacheKeyMetaFactory.getCacheKeyFactory(keyFactoryName);
		if (cacheKeyFactory != null) {
			result = cacheKeyFactory.createCacheLocation(key, pageContext);
		}
		
		// set the cachename if not set in the cacheLocation
		if (result != null && StringUtils.isEmpty(result.getCacheName())) {
	...
	private String getCachedBodyContent(String cacheName, Object cacheKey) {
		Object cachedObject = contentCache.getContent(cacheLocation);
	...
	public int doEndTag() throws JspException {
		...
		contentCache.putContent(cacheLocation, bodyContentAsString);
